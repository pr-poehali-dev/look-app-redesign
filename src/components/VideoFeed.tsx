...
const VideoFeed = ({ activeTab, activeCategory = "all", initialVideoId, onCloseInitial }: VideoFeedProps) => {
  ...
  return (
    <div className="relative w-full h-full look-feed-root" style={{ background: "var(--look-bg)" }}>
      <div
        ref={containerRef}
      ...
      >
        {!dbLoaded ? (
          ...
          return (
            ...
            >
              <div className="w-full md:w-auto md:h-[calc(100%-24px)] h-full" style={{ background: "var(--look-bg)" }}>
      ...
      {/* Desktop nav arrows — у правого края экрана */}
      <div
        className="hidden lg:flex flex-col gap-3 absolute top-1/2 -translate-y-1/2 z-40 right-8"
      >
        <button
